                ...
                <table id="empSched-list" class="table table-striped table-gray" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        <tr v-for="(ob, index) in OB">
                            ...
                            <td><button class="btn btn-link" data-toggle="modal" @click="openModal(ob, index)" data-target="#Info">more...</button></td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="OB">Requested OB by {{imodal.name}}</h5>
        ...
        methods: {
            ...
            submit(index, flag){
                if(this.form.remarks){
                    $("#Info").modal('hide');
                    this.err_msg = "";
                    for(let x = 0; x < this.OB.length; x++){
                        if(this.OB[x].id == index){
                            if(flag == 1){
                                this.form.oth = flag;
                                this.OB[x].status = "Authorized";
                                break;
                            }else{
                                this.form.oth = flag;
                                this.OB[x].status = "Unauthorized";
                                break;
                            }
                        }
                    }
                    this.form.patch('/authorize/forms/ob/' + index);
